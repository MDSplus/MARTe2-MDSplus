...
//                    External start digital trigger on PFI0
//                    Export start trigger to RTSI0
//                    External sample clock from RTSI1
//
...
void test(iBus *bus)
{
    const u32 numberOfChannels = 1;
    const u32 numberOfSamples = 1000;
    ...
    const u32 userSizeInSamples = 1000; 
    ...
    aiTrigger (board,
               tMSeries::tAI_Trigger_Select::kAI_START1_SelectPFI0, // External trigger PFI0
    ...
    aiSampleStart (board, 
                   samplePeriodDivisor, 
                   3, 
                   tMSeries::tAI_START_STOP_Select::kAI_START_SelectRTSI1, // External Sample Clock RTSI1
    ...
    // Export start trigger to RTSI0
    //
    exportSignal (board, kAIStartTrigger, kRTSI0, kTrue);
    ...
    while ( (bytesRead < totalNumberOfBytes) && (kNoError == status))
    {
        ...
        if ( bytesAvailable >= userSizeInBytes )
        {
            ...
            for (u32 i=0; i < userSizeInSamples ; ++i)
            {
                f32 scaled;
    ...
    //
    // Disable export start trigger to RTSI0
    //
    exportSignal (board, kAIStartTrigger, kRTSI0, kFalse);
